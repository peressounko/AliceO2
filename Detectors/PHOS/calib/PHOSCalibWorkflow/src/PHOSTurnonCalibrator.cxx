...
void PHOSTurnonSlot::print() const
{
  for (short ddl = 0; ddl < 14; ddl++) {
    std::array<float, TurnOnHistos::Npt>& all = mTurnOnHistos->getTotSpectrum(ddl);
    std::array<float, TurnOnHistos::Npt>& tr = mTurnOnHistos->getTrSpectrum(ddl);
...
void PHOSTurnonCalibrator::endOfStream()
{
  ...
  for (int ddl = 0; ddl < TurnOnHistos::NDDL; ddl++) {
    ...
    std::array<float, TurnOnHistos::Npt>& vf = mTurnOnHistos->getTrSpectrum(ddl);
    std::array<float, TurnOnHistos::Npt>& va = mTurnOnHistos->getTotSpectrum(ddl);
    for (int i = 0; i < 200; i++) {
