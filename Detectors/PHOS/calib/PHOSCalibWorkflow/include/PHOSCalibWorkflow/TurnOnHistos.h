...
class TurnOnHistos
{
 public:
  ...
  void fillFiredMap(std::bitset<NCHANNELS>& bs)
  {
  ...
  /// \param bitset with channels fired in event
  void fillNoisyMap(std::bitset<NCHANNELS>& bs)
  {
    for (short i = NCHANNELS; --i;) {
      if (bs[i]) {
        mNoisyMap[i]++;
  ...
  //getters now
  std::array<float, Npt>& getTotSpectrum(short ddl) { return mTotSp[ddl]; }
  std::array<float, Npt>& getTrSpectrum(short ddl) { return mTrSp[ddl]; }
  std::array<float, NCHANNELS> getGoodMap() { return mGoodMap; }
  std::array<float, NCHANNELS> getNoisyMap() { return mNoisyMap; }
